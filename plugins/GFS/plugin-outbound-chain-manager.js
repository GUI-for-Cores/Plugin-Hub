const PATH = 'data/third/outbound-chain-manager'
const GROUP_OUTBOUND_TYPES = new Set(['selector', 'urltest'])
const EXCLUDED_SOURCE_OUTBOUND_TYPES = new Set(['direct', 'block', 'dns'])

/** @type {EsmPlugin} */
export default (Plugin) => {
  /* 触发器 生成配置时 */
  const onGenerate = async (config, profile) => {
    const context = await buildSingBoxChainContext(config, profile)
    ...
    const detourByTag = buildDetourMap(chainConfig.rules, context)
    if (detourByTag.size === 0) return config

    for (const outbound of config.outbounds || []) {
      if (!outbound?.tag) continue
      const detourTag = detourByTag.get(outbound.tag)
      if (detourTag && detourTag !== outbound.tag) {
  ...
  const showUI = async (profile) => {
    ...
    const config = await Plugins.generateConfig(profile, { enablePluginProcessing: false })
    const context = await buildSingBoxChainContext(config, profile)
    const filePath = `${PATH}/${profile.id}.json`
    const chainConfig = normalizeChainConfig(
      await Plugins.ReadFile(filePath)
    ...
    const outboundOptions = toOptions([...context.outboundByTag.keys()])
    const groupOptions = toOptions([...context.groupByName.keys()])
    const initialRules = chainConfig.rules.map((rule) => normalizeRule(rule, outboundOptions, groupOptions, context))
    ...
    const previewOutboundNodeStyle =
    ...
    <div class="flex flex-col gap-8 pb-8 pr-8" style="width: min(820px, 100%); margin: 0 auto;">
      ...
      <div class="grid items-center gap-8" style="grid-template-columns: minmax(0, 1fr) auto;">
        ...
        <input v-model="previewKeyword" class="text-12" :style="searchInputStyle" placeholder="搜索链路、出站或规则" />
      </div>
      <Card>
            ...
            <div class="flex items-center gap-6">
              ...
              <div class="flex items-center min-w-0 flex-1" style="padding: 2px 0;">
                <template v-for="(part, index) in item.parts" :key="part.name + '-' + index">
                  ...
                  <div class="text-12" :title="part.name" :style="part.type === 'group' ? previewGroupNodeStyle : previewOutboundNodeStyle">
    ...
    `,
      setup() {
        ...
        watch(
          rules,
          (items) => {
            items.forEach((rule) => ensureRuleSelection(rule, outboundOptions, groupOptions, context))
          },
          { deep: true }
        )
        ...
        const openRuleEditor = (index) => {
          ...
          const draft = ref(cloneRule(isEdit ? rules.value[index] : createRule(outboundOptions, groupOptions, context)))
          ...
          `,
            setup() {
              watch(
                draft,
                () => {
                  ensureRuleSelection(draft.value, outboundOptions, groupOptions, context)
                },
                { deep: true }
              )
              ...
              const openEntityPicker = (field) => {
                ...
                const outboundList = computed(() => filterEntityOptions(outboundOptions, keyword.value))
                ...
                <div class="flex flex-col gap-8" style="width: min(520px, 100%); margin: 0 auto; padding: 8px; border-radius: 4px; background: #f8fafc;">
                  <input v-model="keyword" placeholder="搜索出站分组或出站" style="height: 30px; padding: 0 10px; border: 1px solid #cbd5e1; border-radius: 4px; background: #ffffff; outline: none;" />
                  <div class="flex flex-col gap-10" style="max-height: 360px; overflow: auto;">
                    <div class="flex flex-col gap-6">
                      <div class="font-bold text-13">出站分组</div>
                      <div v-if="groupList.length === 0" class="text-12 text-gray-500">无匹配出站分组</div>
                    ...
                    <div class="flex flex-col gap-6">
                      <div class="font-bold text-13">出站</div>
                      <div v-if="outboundList.length === 0" class="text-12 text-gray-500">无匹配出站</div>
                      <button v-for="item in outboundList" :key="'outbound-' + item.value" type="button" :title="getEntityOptionTitle('proxy', item.value)" :style="getEntityOptionStyle('proxy', item.value)" @click="chooseEntity('proxy', item.value)">
                        {{ item.label }}
                      </button>
                    </div>
                  </div>
          ...
          const editorModal = Plugins.modal(
            {
              ...
              maskClosable: true,
              async onOk() {
                const normalized = normalizeRule(draft.value, outboundOptions, groupOptions, context)
        ...
        const renderRuleMatchSummary = (rule) => {
          ...
          const normalized = normalizeRule(rule, outboundOptions, groupOptions, context)
          ...
          const count = resolveRuleSourceOutbounds(normalized, context).length
          return count > 0 ? `命中 ${count} 个出站` : '未命中现有出站'
        }
    ...
    const modal = Plugins.modal(
      {
        ...
        async onOk() {
          const normalizedRules = rules.value
            .map((rule) => normalizeRule(rule, outboundOptions, groupOptions, context))
            .filter((rule) => isValidRule(rule, context))
  ...
  const buildSingBoxChainContext = async (config, profile) => {
    ...
    const registerOutbound = (outbound) => {
      if (!outbound?.tag || !isSourceOutbound(outbound)) return
      outboundByTag.set(outbound.tag, outbound)
      registerAlias(outbound.tag, outbound.id)
    }

    const registerGroup = (group) => {
      if (!group?.tag || !isGroupOutbound(group)) return
      groupByName.set(group.tag, normalizeGroupMembers(group, nameById))
      registerAlias(group.tag, group.id)
    }

    for (const outbound of profile.outbounds || []) {
      registerAlias(outbound.tag, outbound.id)
      for (const member of outbound.outbounds || []) {
        registerAlias(member.tag, member.id)
      }
    }

    for (const outbound of config.outbounds || []) {
      registerAlias(outbound.tag, outbound.id)
      registerGroup(outbound)
  ...
  const normalizeGroupMembers = (group, nameById) => {
    return {
      ...group,
      members: collectGroupMemberTags(group, nameById)
    }
  }

  const collectGroupMemberTags = (group, nameById) => {
    const members = []
    for (const member of group.outbounds || []) {
      if (typeof member === 'string') {
        members.push(nameById.get(member) || member)
      } else if (member?.tag) {
        members.push(member.tag)
      } else if (member?.id) {
        members.push(nameById.get(member.id) || member.id)
      }
    }
  ...
  const isGroupOutbound = (outbound) => {
    return GROUP_OUTBOUND_TYPES.has(outbound?.type)
  }

  const isSourceOutbound = (outbound) => {
    return outbound?.tag && !isGroupOutbound(outbound) && !EXCLUDED_SOURCE_OUTBOUND_TYPES.has(outbound.type)
  }
  ...
  const normalizeRule = (rule = {}, outboundOptions, groupOptions, context) => {
    ...
    const sourceName = resolveEntityName(rule.sourceName, sourceType, context) || getDefaultName(sourceType, outboundOptions, groupOptions)
    const targetName = resolveEntityName(rule.targetName, targetType, context) || getDefaultName(targetType, outboundOptions, groupOptions)
  ...
  const createRule = (outboundOptions, groupOptions, context) => {
    const sourceType = groupOptions.length > 0 ? 'group' : 'proxy'
    const targetType = outboundOptions.length > 0 ? 'proxy' : 'group'
    return normalizeRule({ sourceType, targetType }, outboundOptions, groupOptions, context)
  }
  ...
  const renderRuleSummary = (rule) => {
    const sourceType = rule.sourceType === 'proxy' ? '出站' : '出站分组'
    const targetType = rule.targetType === 'proxy' ? '出站' : '出站分组'
  ...
  const renderEntityLabel = (type, name) => {
    if (!name) return '请选择'
    return `${type === 'proxy' ? '出站' : '出站分组'}: ${name}`
  }
  ...
  const getRuleStatusMessages = (rule, context) => {
    ...
    const count = collectLeafOutboundTags(normalized.sourceName, context, normalized.recursive !== false).length
    return count > 0 ? [] : [`出站分组「${normalized.sourceName}」未识别到可设置上游的出站`]
  }

  const ensureRuleSelection = (rule, outboundOptions, groupOptions, context) => {
    ...
    if (!hasOption(rule.sourceName, getOptions(rule.sourceType, outboundOptions, groupOptions))) {
      rule.sourceName = getDefaultName(rule.sourceType, outboundOptions, groupOptions)
    }
    if (!hasOption(rule.targetName, getOptions(rule.targetType, outboundOptions, groupOptions))) {
      rule.targetName = getDefaultName(rule.targetType, outboundOptions, groupOptions)
    }
  ...
  const getDefaultName = (type, outboundOptions, groupOptions) => {
    return type === 'group' ? groupOptions[0]?.value : outboundOptions[0]?.value
  }

  const getOptions = (type, outboundOptions, groupOptions) => {
    return type === 'group' ? groupOptions : outboundOptions
  }
  ...
  const buildSourceUsage = (rules, context, options = {}) => {
    ...
    for (const [index, rule] of (rules || []).entries()) {
      ...
      for (const nodeName of resolveRuleSourceOutbounds(normalized, context)) {
  ...
  const findRuleSourceConflict = (rule, rules, context, currentIndex = -1, options = {}) => {
    ...
    const sourceNodes = resolveRuleSourceOutbounds(normalized, context)
  ...
  const findSourceEntityBlocker = (type, name, rules, context, currentIndex = -1) => {
    ...
    const nodes = type === 'group' ? collectLeafOutboundTags(sourceName, context, true) : [sourceName]
  ...
  const buildSourceConflict = (rule, owner, nodeName = '') => {
    return {
      ...
      owner,
      nodeName,
      message: nodeName ? `来源冲突：出站「${nodeName}」已被 ${owner.label} 命中` : `来源冲突：该来源已被 ${owner.label} 使用`
    }
  }
  ...
  const buildSourceBlocker = (owner, nodeName = '') => {
    return {
      owner,
      nodeName,
      message: nodeName ? `不可选择：组内出站「${nodeName}」已被 ${owner.label} 命中` : `不可选择：已被 ${owner.label} 作为来源`
    }
  }

  const renderSourceConflictMessage = (conflict) => {
    return conflict?.message || '同一个出站只能配置一个上游'
  }

  const entityExists = (type, name, context) => {
    const resolvedName = resolveEntityName(name, type, context)
    return type === 'group' ? context.groupByName.has(resolvedName) : context.outboundByTag.has(resolvedName)
  }
  ...
  const inferEntityType = (value, context, fallback) => {
    ...
    if (context.outboundByTag.has(name)) return 'proxy'
    return fallback
  }
  ...
  const resolveEntityName = (value, type, context) => {
    ...
    if (type === 'proxy' && context.outboundByTag.has(name)) return name
    return name
  }
  ...
  const buildDetourMap = (rules, context) => {
    return buildDetourContext(rules, context).detourMap
  }

  const buildDetourContext = (rules, context) => {
    ...
    for (const rule of rules || []) {
      ...
      const sources = resolveRuleSourceOutbounds(normalizedRule, context)
  ...
  const setChainLink = (links, ruleIds, sourceName, targetName, ruleId) => {
    ...
    if (!hasDetourCycle(sourceName, links)) return true
  ...
  const buildPreviewItems = (rules, context) => {
    const { detourMap, previewMap, previewRuleIds } = buildDetourContext(rules, context)
  ...
  const resolveRuleSourceOutbounds = (rule, context) => {
    if (rule.sourceType === 'proxy') {
      const sourceName = resolveEntityName(rule.sourceName, 'proxy', context)
      return context.outboundByTag.has(sourceName) ? [sourceName] : []
    }
    return collectLeafOutboundTags(rule.sourceName, context, rule.recursive !== false)
  }

  const collectLeafOutboundTags = (groupName, context, recursive = true, seen = new Set()) => {
    ...
    if (!group) return context.outboundByTag.has(resolvedGroupName) ? [resolvedGroupName] : []

    const result = []
    for (const name of group.members || []) {
      const resolvedName = resolveAnyEntityName(name, context)
      if (context.outboundByTag.has(resolvedName)) {
        result.push(resolvedName)
      } else if (recursive && context.groupByName.has(resolvedName)) {
        result.push(...collectLeafOutboundTags(resolvedName, context, recursive, new Set(seen)))
      }
    }
  ...
  const hasDetourCycle = (source, detourMap) => {
    const seen = new Set()
    let current = source

    while (detourMap.has(current)) {
      if (seen.has(current)) return true
      seen.add(current)
      current = detourMap.get(current)
    }

    return false
  }

  const renderDetourChain = (source, detourMap) => {
    return getChainParts(source, detourMap).join(' -> ')
  }

  const getChainParts = (source, detourMap) => {
    const parts = [source]
