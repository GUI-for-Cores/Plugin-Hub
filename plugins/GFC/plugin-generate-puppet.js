const addProfile = async () => {
  const url = await Plugins.prompt('请输入你的配置文件路径，例如：C:\\Users\\Admin\\Desktop\\config.yaml')
  if (!url || !url.trim()) {
    throw '缺少配置文件路径'
  }
  ...
  if (config['external-controller']) {
    profile.advancedConfig['external-controller'] = config['external-controller']
  }
  if (config['secret']) {
  ...
  plugin.description = '配置：' + name + '的对应插件，用于在生成配置文件时使用自定义的配置文件。'
...
const delProfile = async () => {
  ...
  const ids = await Plugins.picker.multi(
    ...
    profilesStore.profiles.filter((v) => v.name.startsWith('[傀儡配置]')).map((v) => ({ label: v.name, value: v.id })),
    []
  )
  const pluginsStore = Plugins.usePluginsStore()
