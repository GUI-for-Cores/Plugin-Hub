...
export default (plugin) => {
  ...
  const runtimeState = { uiSubId: null, automationSubIds: new Set() }
  ...
  const loadJson = async (filePath, defaultValue) => {
    ...
    return Array.isArray(defaultValue) ? JSON.parse(content) : { ...defaultValue, ...JSON.parse(content) }
  }
  ...
  const testSingleProxy = async (ctx, proxy, settings) => {
    ...
    return { tag, type, delay, ipv6: ipv6Delay > 0 }
  }
  ...
  const testAllProxies = async (ctx, proxies, settings, options) => {
    ...
    const { run, controller } = Plugins.createAsyncPool(settings.concurrencyLimit, proxies, async (p) => {
      ...
      index += 1
      if (!silent) {
        updateUI?.(updateMsgText('测试中...'))
      }
    })

    let updateUI = undefined
    let destroyUI = undefined
    let successUI = undefined
    if (!silent) {
  ...
  const runCore = async (targetConfigPath, subDir) => {
    ...
    const core = Plugins.getKernelFileName(isAlpha)
  ...
  const stopRuntimeContext = async (subId) => {
    ...
    if (ctx.pid) {
      await Plugins.KillProcess(ctx.pid).catch(() => {})
    }
    if (ctx.configPath) {
      await Plugins.RemoveFile(ctx.configPath).catch(() => {})
    }
    runtimeContextMap.delete(subId)
  }

  const executeScript = (code, proxies) => {
    const fn = new globalThis.AsyncFunction('proxies', `${code}; return operate(proxies)`)
    return fn(proxies)
  }

  const persistUIStateOnClose = async (subscription, uiProxies) => {
    const sorted = [...uiProxies].toSorted((a, b) => {
  ...
  const openUI = (subscription, ctx) => {
    ...
    const subConfig = ref({ ...DEFAULT_SUBSCRIPTION_CONFIG })
    ...
    const sortedProxies = computed(() =>
      [...uiProxiesList.value].toSorted((a, b) => {
        const valA = a.delay <= 0 ? Infinity : a.delay
        const valB = b.delay <= 0 ? Infinity : b.delay
        return valA - valB
      })
    ...
    const deleteNode = async (proxy) => {
      if (!(await Plugins.confirm('提示', `确定要删除 [${proxy.tag}] 吗？`).catch(() => false))) {
        return
      }
    ...
    const cleanNodes = async (threshold) => {
      ...
      if (!(await Plugins.confirm('清理节点', `确定要删除所有${text}的节点吗？`).catch(() => false))) {
        return false
      }
    ...
    const openAutomationUI = () => {
      const draft = ref({ ...subConfig.value.automation })
      ...
      const modal = Plugins.modal({
        ...
        cancelText: '取消',
        onOk: async () => {
          subConfig.value.automation = { ...draft.value }
          await SubConfigStore.save(subConfig.value, subscription.id)
          Plugins.message.success('已保存')
          return true
        }
    ...
    const openCleanUI = () => {
      const component = defineComponent({
        ...
        setup(_, { expose }) {
          expose({
            modalSlots: {
              action: () => h('div', { class: 'mr-auto text-12' }, '输入 0 仅清理测试失败的节点')
            }
          })
