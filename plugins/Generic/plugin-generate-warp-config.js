...
const DEFAULT_OPENSSL_PATH = 'C:/Program Files/FireDaemon OpenSSL 3/bin/openssl.exe'
...
export default (Plugin) => {
  ...
  const generateWarpConfig = async () => {
    const openSslExecPath =
      envStore.env.os === 'windows' ? Plugin.OpenSslExecPath.replace(/\\/g, '/').replace(/^"|"$/g, '') || DEFAULT_OPENSSL_PATH : '/usr/bin/openssl'
  ...
  const generateConfigScript = async (warpConfig) => {
    ...
    let upStreamProxy = ''
    try {
      proxyGroups = await Plugins.prompt('请输入要包含 Warp 节点的代理组/出站分组名称，多个分组请用英文逗号隔开', '', {
        placeholder: 'Group1,Group2,Group3',
        type: 'text'
      })
      upStreamProxy = await Plugins.prompt('请输入要作为前置代理的节点/代理组名称，如果不需要请留空', '', { placeholder: 'Proxy1', type: 'text' })
    } catch (e) {
      throw '未输入有效代理名称'
    }
    let configScript = ''
