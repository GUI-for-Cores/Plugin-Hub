...
const Ask = async () => {
  ...
  8. 计划任务使用开源库robfig/cron，表达式支持6位，例如：* * * * * *。为用户生成cron表达式时请使用6位。
...
const myPrompt = async (placeholder, system_instruction) => {
  ...
  const { status, body } = await Plugins.HttpPost(
    `https://api.cloudflare.com/client/v4/accounts/${Plugin.ACCOUNT_ID}/ai/run/@cf/meta/llama-3-8b-instruct`,
    {
  ...
  if (status !== 200) {
    ...
    throw ErrorMap[body.errors[0]?.code] || body.errors[0]?.message
  }

  const question = input.endsWith('?') || input.endsWith('？') ? input : input + '？'

  return [question, body.result.response || 'Error: \n\n' + JSON.stringify(body, null, 2)]
}
...
const checkApiKey = async () => {
  ...
  if (!Plugin.ACCOUNT_ID) {
    await Plugins.alert(Plugin.name, '请在插件中填写你的账户ID')
    return false
  }
  return true
}
