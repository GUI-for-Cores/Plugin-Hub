...
const ExportToURI = async (subscription) => {
  const { path } = subscription
  const isJSON = path.endsWith('.json')
  const isYAML = path.endsWith('.yaml')
  let proxies_full_field
  const content = await Plugins.Readfile(path)
  if (isJSON) {
    proxies_full_field = JSON.parse(content)
  } else if (isYAML) {
    proxies_full_field = Plugins.YAML.parse(content)
  } else {
    throw '不支持的节点类型'
  }
  const { produce } = URI_Producer()
  const uriList = proxies_full_field.map((proxy) => produce(proxy))
  console.log(`[${Plugin.name}]`, uriList)
}
